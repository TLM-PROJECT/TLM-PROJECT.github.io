...
puntos <- sf::st_read("//157.88.237.17/tlm_project2/1_TLS_DRON/data/buffer_12m_centros.shp")
st_crs(puntos) <- 25830 
puntos_centroides <- st_centroid(puntos)
provincias_utm <- mapSpain::esp_get_prov() %>% st_transform(25830)
provs <- c("Palencia", "Burgos", "Soria", "Guadalajara", "Zaragoza", "Huesca", "Navarra", "La Rioja")
provincias_estudio <- provincias_utm[provincias_utm$prov.shortname.es %in% provs, ]
#provincias_estudio <- provincias_utm[provincias_utm$nuts2.name == "Aragon", ]
...
library(patchwork) # Para combinar mapas

mapa_referencia <- ggplot() +
  geom_sf(data = provincias_utm, fill = NA, color = "grey60", size = 0.3) + 
  geom_sf(data = provincias_estudio, fill = "red", alpha = 0.5, color = "red") +
  #coord_sf(xlim = c(-15, 10), ylim = c(34, 55)) + # Zoom a la zona
...
mapa_principal <- ggplot() +
  ...
  geom_sf(data = provincias_estudio, fill = NA, color = "grey60", size = 0.4) + # Límites provincias
  geom_sf_text (data = provincias_estudio, aes(label=ine.prov.name)) +
...
mapa_principal + 
  inset_element(mapa_referencia, 
                left = 0.70,   # Empieza pasando la mitad (60%)
                bottom = 0.0, # Pegado al borde inferior
                right = 1.0,  # Termina en el borde derecho
                top = 0.30)    # Sube hasta el 40% de la altura
```
